...
export default function Wishlist() {
  ...
  const getwish = () => {
    axios
      .get("https://calm-cyan-octopus-wear.cyclic.app/wishlist", {
        headers: {
  ...
  const deleteWish = async (id) => {
    await fetch(
      `https://calm-cyan-octopus-wear.cyclic.app/wishlist/delete/${id}`,
      {
        method: "DELETE",
        headers: {
          Authorization: localStorage.getItem("token"),
    ...
    <div>
      ...
      <div className="wishmain">
        {data.length > 0 &&
          data.map((el) => {
            return (
              <div key={el.productId._id} className="wishmap">
                <button
                  onClick={() => deleteWish(el._id)}
                  className="wishfirstbtn"
                >
                  X
                </button>
                <div className="wishaboveimg">
                  ...
                  <div className="wishlistlbimg">
                    <div className="wishlistrate">
                      <span className="wishlisttetext">
                        {el.productId.rating}
                      </span>
                      <span>
                        <AiFillStar className="wishlisticon" />
                      </span>
                    </div>
                    <br />
                  ...
                  <button
                    onClick={() => abc(el.productId._id, el._id)}
                    className="wishbtn"
                  >
                    Move to Bag
                  </button>
